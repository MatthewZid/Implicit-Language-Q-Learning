...
import random
from socraticdial.socraticdial_base import AnswerEvent, CutoffRule, Event, QuestionEvent, Scene, N_TURNS, StopEvent
from socraticdial.socraticdial_base import yn_reward_fs

class SDObservation(Language_Observation):
...
class SDEnvironment(Language_Environment):
    def __init__(self, dataset: RL_Dataset, url: str, reward_shift: float=0.0, 
        ...
        self.remote_env = SDEnvRemoteWrapper(url)
    ...
    def step(self, action: str) -> Tuple[SDObservation, float, bool]:
    ...
    def reset(self) -> SDObservation:
        ...
        self.state = SDObservation(scene)
        return self.state

    def is_terminal(self) -> bool:
        return self.state.event is not None and self.state.event.is_final()

class SDEnvRemoteWrapper:
    def __init__(self, url: str) -> None:
        self.url = url

    def step(self, obs: SDObservation):
        history = []
...
class SDRemotePolicy(Policy):
    def __init__(self, url: str) -> None:
        self.url = url

    def act(self, obs: SDObservation):
        history = []
        if obs.event is not None:
            for item in obs.event.get_events():
                if isinstance(item, QuestionEvent):
...
class SDRemoteReward:
    def __init__(self, url: str) -> None:
        self.url = url

    def reward(self, obs: SDObservation):
        history = []
        if obs.event is not None:
            for item in obs.event.get_events():
                if isinstance(item, QuestionEvent):
